...
Эта процедура построена на базе фреймворка OAuth2 [OAuth 2.0]:http://oauth.net/2/
Почитать об OAuth 2.0 простым и понятным языком можно [тут]:http://habrahabr.ru/company/mailru/blog/115163/

# Регистрация вашего приложения

Прежде всего необходимо зарегистрировать ваше приложение или сервис на странице http://goodgame.ru/user/settings/oauth2
...
* Redirect URI - URI куда будет перенаправлен пользователь Goodgame, после того как подвердит авторизацию вашего приложения на использование своего аккаунта. Если оставить пустым, то перенаправление будет на служебную страницу http://api2.goodgame.ru/oauth/recievecode
* Client Secret - пароль для вашего идентификатора (должны знать только вы).

Для получения доступа к защищенному API Goodgame ваше приложение или сервис должны получить `access_token` - идентификатор, подтверждающий что пользователь дал согласие выполнять запросы к от его имени.
...
Используя наше API вы можете получить __access token__ используя следующие 3 шага.
...
`state` - это любой сгенирированный вами идентификатор, необходим для избежания CSRF-атак на ваше приложение. Передается в неизменном виде при редиретке на ваш Redirec URI.
...
После подвержедения пользователем авторизации вашего приложения, он будет перенаправлен на страницу
...
Этот сценарий является довольно распространненым, например в Javascript-приложениях, когда вы не можете безопасно хранить свой __Client Secret__.
...
После подвержедения пользователем авторизации вашего приложения, он будет перенаправлен на страницу __Redirect URI__, используя URI-фрагмент #access_token в качестве __Access Token__
...
Указывается только те scope которые действительно нужны вашему приложению.
